...
export function showSpinner(container: HTMLElement): void {
	// Add awaitingResponse class
	container.classList.add('awaitingResponse');
 ...
 */
export function hideSpinner(container: HTMLElement | null): void {
	// Remove spinner element
	const spinner = container?.querySelector('.spinner');
	if (spinner) {
		spinner.remove();
		container?.classList.remove('awaitingResponse');
	}
}
 ...
 * @param count - The count parameter is the number of results to retrieve from the REST API.
