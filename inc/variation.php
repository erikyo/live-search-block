...
function register_search_block_variation() {
	...
	$variation_render_callback = function ( $attributes, $content, $block ) {
		...
		$attributes = wp_parse_args(
			$attributes,
			array(
				'label'      => __( 'Search', 'live-search-block' ),
				'buttonText' => __( 'Search', 'live-search-block' ),
			)
		);
		...
		$label_inner_html = empty( $attributes['label'] ) ? __( 'Search', 'live-search-block' ) : wp_kses_post( $attributes['label'] );
	...
	$cloned_block->title    = __( 'Live Search', 'live-search-block' );
	$cloned_block->category = 'widgets';
